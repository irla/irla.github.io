...
export const Project: React.FC<ProjectProps> = ({project, left, filter}) => {
    if (project) {
        const pointer = left 
            ? <div className="hidden sm:block relative rotate-45 w-2 h-2 border-r border-t z-10 bg-slate-50 -right-1"></div>
            : <div className="hidden sm:block relative rotate-45 w-2 h-2 border-l border-b z-10 bg-slate-50 -left-1"></div>

        return (
        <div className={"border rounded-md flex flex-wrap my-1 " + (left ? "sm:mr-2" : "sm:ml-2")} >
            <div className={"basis-full bg-slate-50 flex items-center py-1 border-b " + (left ? "text-right" : " flex-row-reverse")}>
